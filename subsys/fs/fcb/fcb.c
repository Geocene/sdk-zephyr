...
LOG_MODULE_REGISTER(FCB_INT, LOG_LEVEL_INF);
...
fcb_init(int f_area_id, struct fcb *fcb)
{
	...
	struct flash_sector oldest_sector, newest_sector;
	...
	if ((fcb->f_sector_end - fcb->f_sector_start != (fcb->f_sector_cnt - 1)) || 
